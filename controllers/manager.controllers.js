...
const staticAllProduct = async (req, res) => {
    ...
    try {
        let namespace = "Cơ sở sản xuất";
        if(req.query.namespace == 'sv') namespace = "Trung tâm bảo hành"
        if (req.query.namespace == 'ag') namespace = "Đại lý phân phối"
        ...
        const products = await product.find({namespace: namespace});
        for (let i = 0; i < products.length; i++) {
            const product_ = await listProduct.findOne({name: products[i].name});
            let id_ = products[i].id_pr;
            if (namespace == "Đại lý phân phối") id_ = products[i].id_ag;
            if (namespace == "Trung tâm bảo hành") id_ = products[i].id_sv;
